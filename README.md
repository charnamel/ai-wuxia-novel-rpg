...
电脑端：左侧是分组的命令导航，中间是剧情阅读区，底部输入行动。

<p align="center">
  <img src="assets/screenshots/desktop.png" alt="电脑端界面" width="820">
</p>

手机端：命令收纳进底部抽屉，点左下角「☰ 命令」展开，弹窗自动全屏。

<p align="center">
  <img src="assets/screenshots/mobile.png" alt="手机端界面" width="290">
</p>
...
- 主线按原著桥段推进，也可以跳着玩
- 网页界面游玩，电脑手机都适配，带地图、任务、装备等管理面板
- 结构化输入：`@角色` 写 NPC 的戏、`#旁白` 写环境，AI 不会搞混谁在说谁在做
...
想了解界面上每个按钮干什么、对战/存档/调试指令怎么写，看[网页界面与指令指南](docs/05-网页界面与指令指南.md)。

## 结构化输入：@角色 / #旁白

默认你写的都算「玩家自己的言行」。想写旁白、或给 NPC 安排戏，用两个符号标归属：

| 写法 | 含义 |
|---|---|
| `@角色名：内容` | 由该角色演出（台词/心理/动作交给 AI 按文义判断）|
| `#内容` | 旁白 / 环境叙述 |
| 不加标记 | 玩家自己的言行（默认）|
...
- 输入框旁有 **`@`、`#` 快捷按钮**；打 `@` 会弹出 NPC 候选（按「在场 > 临时登场 > 全量」排序），点选即插入。
- `@` 的名字自动匹配 NPC 名单，**名单外也支持**（用 `：` 或 `，` 与内容隔开，如 `@黑衣人：你等着`）。
- 想打**字面** `@`/`#`，写 `\@`、`\#`。
- 地图选了目标地点后，系统会自动注入 `【场景目标】`，不用手写。

好处：AI 不会把你写的 NPC 台词/心理误当成玩家说的话；你也可以像导演一样排 NPC 的戏。详见[网页界面与指令指南](docs/05-网页界面与指令指南.md)。
...
- [网页界面与指令指南](docs/05-网页界面与指令指南.md)
