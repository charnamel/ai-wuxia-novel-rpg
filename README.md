...
- 剧本红线：把"不许发生的事"写成既定事实，每轮注入，AI 不会往那儿走
...
剧情跑偏了？打开左侧「百科管理 → 剧本红线」，写几条**红线条目**。它会作为「当前既定事实」每轮注入，AI 会绕开这些走向。

| 写法 | 例 |
|---|---|
| ✅ 正面事实 | `陆菲青不死不伤`、`本世界无鬼怪、无魔教` |
| ❌ 否定句 | `不要让陆菲青死`、`禁止出现魔教`（模型容易"念"出来，反而把雷点带进剧情）|

- 支持增 / 删 / 清空，最多 30 条、单条 80 字；**列表为空时完全不注入**（不占 token）
- 存在 `data/plot_guard.json`，**跟着存档走**（换档/读档连带切换），加删后**下一轮**生效
- 反过来想"强行让某件事发生"：输入以 `！！` 开头，本轮剧情会按你的指令强制演绎（最高优先级）
