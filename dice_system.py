...
# V5.2: 撤销 V5.1 的 -6 补偿（实测偏离设计意图，1234 档偏松）：1/2/3 档阈值回调
#       1档≥18 / 2档14~17 / 3档7~13 / 4档0~6，4~8 档区间不动
...
_DELTA_GRADE_TABLE = [
    ...
    (7,     3, "正常发挥", "得心应手,招式熟练,符合自身预期水准,一切按部就班"),
...
def judge_grade_v4(delta: int, natural: int,
    ...
    # V5.2放宽: delta 达标（≥18 / ≤-18）同样可保持——否则阈值提到 18 后 1/8 档实际只剩 nat/碾压专属
    # 1档: natural=20 或 玩家境界碾压对手6档以上 或 delta≥18 时保持, 否则降为2档
    if final_grade == 1 and natural != 20 and not _player_dominates and delta < 18:
        final_grade = 2
    # 8档: natural=1 或 对手境界碾压玩家6档以上 或 delta≤-18 时保持, 否则升为7档
    if final_grade == 8 and natural != 1 and not _opponent_dominates and delta > -18:
        final_grade = 7
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "environment_mod": {
                    ...
                    "description": "天时地利与战术态势(对玩家有利为负)：天时不利+1·开阔有利-1·偷袭得手-1~-2·玩家群战围杀NPC-1~-3·玩家以一敌多NPC+1~+3。无则0",
                },
                "environment_reason": {
                    "type": "string",
                ...
                "situation_mod": {
                    ...
                    "description": "人和战况(对玩家有利为负)：对手负伤-1~-3·对手受制-1~-3·玩家受制+1~+3·心神不宁+1~+2。HP≤70%按负伤·MP=0（内力枯竭）按受制·玩家HP≤70%或MP=0按带伤。无则0",
                },
                "situation_reason": {
                    "type": "string",
...
■ 第一类：天时地利与战术态势（加减总封顶 -4 ~ +4，玩家有利为减）
  减值（DC -）：开阔有利 -1；偷袭得手 -1~-2；玩家一方群战围杀 -1~-3
  加值（DC +）：天时不利（黑暗/雨雪湿滑/酷暑严寒/大风）+1~+2；玩家以一敌多 +1~+3

■ 第二类：人和战况（加减总封顶 -3 ~ +3，玩家有利为减）
  减值（DC -）：对手负伤 -1~-3；对手受制（中毒/封穴/内力枯竭）-1~-3
