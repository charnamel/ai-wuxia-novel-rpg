#!/usr/bin/env python3
"""
数眼科技 doubao-seed-1-6-flash-250828 NPC头像生成脚本
通过 platform.shuyanai.com 中转调用豆包多模态模型图片生成

用法 (在项目根目录运行):
  python tools/generate_npc_avatar_shuyan.py <NPC姓名> "<提示词>"

示例:
  python tools/generate_npc_avatar_shuyan.py 血刀老祖 "Chinese traditional colorful ink wash painting portrait of a fierce Tibetan monk"

也可以批量生成:
  python tools/generate_npc_avatar_shuyan.py --batch npc_list.json
"""
...
# ============ 配置区 ============
API_URL = "https://platform.shuyanai.com/v1/images/generations"
API_KEY = "sk-16jlBSI3c5FOztEskfzzDWACeGJrqI4SSwOUhjmOXWeN35XT"
...
def call_api(prompt):
    """调用数眼科技图片生成API，返回图片二进制数据（失败自动重试）"""
    headers = {
    ...
    # 兼容两种返回格式：b64_json（base64）和 url（下载链接）
...
def main():
    ...
    if sys.argv[1] == "--batch":
        if len(sys.argv) < 3:
            print("用法: python generate_npc_avatar_shuyan.py --batch <json文件路径>")
            sys.exit(1)
        batch_generate(sys.argv[2])
    else:
        if len(sys.argv) < 3:
            print("用法: python generate_npc_avatar_shuyan.py <NPC姓名> <提示词>")
            sys.exit(1)
        name = sys.argv[1]
        prompt = sys.argv[2]
